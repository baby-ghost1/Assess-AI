...
function FloatingInput({ label, icon: Icon, error, registration, type, showToggle, showState, onToggleShow }) {
  ...
  const handleFocus = () => {
    setFocused(true)
    gsap.to(iconRef.current, { scale: 1.2, color: '#06B6D4', duration: 0.3, ease: 'back.out(1.7)' })
    gsap.to(glowRef.current, { opacity: 1, scaleX: 1, duration: 0.4, ease: 'power2.out' })
    gsap.to(wrapperRef.current, { borderColor: 'rgba(6, 182, 212, 0.5)', duration: 0.3 })
  }
    ...
    <div className="form-field">
      ...
      >
        ...
        <div className="relative overflow-hidden rounded-b-[11px]">
          ...
          <div
            ref={glowRef}
            className="absolute bottom-0 left-0 right-0 h-[2px] bg-gradient-to-r from-accent to-primary opacity-0 origin-center"
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.from(formFields, {
        y: 25, opacity: 0, stagger: 0.07, duration: 0.4, delay: 0.5, ease: 'power2.out',
      })

      if (dividerRef.current) {
        gsap.from(dividerRef.current, { scaleX: 0, opacity: 0, duration: 0.5, delay: 1.1, ease: 'power2.out' })
      }
        ...
        <div ref={cardRef} className="w-full max-w-md">
          ...
          <div className="relative rounded-2xl p-[1px] overflow-hidden">
            <div
              ...
              style={{
                background: 'linear-gradient(135deg, #27272A, #06B6D4, #4F46E5, #27272A)',
