      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {attempts.map((a) => {
            ...
            const pending = inProgress ? false : a.resultReleased === false
            const passed = !pending && a.percentage >= (a.assessment?.passingPercentage || 40)
            const canRetake = a.isUnlimited || Number(a.remainingAttempts ?? 0) > 0
            const scoreColor = pending ? 'text-warning' : (passed ? 'text-success' : 'text-danger')
            const badge = pending ? { label: 'Pending', cls: 'bg-warning/10 text-warning' } : inProgress ? { label: 'In Progress', cls: 'bg-warning/10 text-warning' } : { label: passed ? 'Passed' : 'Failed', cls: passed ? 'bg-success/10 text-success' : 'bg-danger/10 text-danger' }
            return (
              <div key={a._id} className="rounded-xl border border-border bg-bg-card p-5 hover:shadow-md transition-all">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    <div className={`shrink-0 h-10 w-10 rounded-lg flex items-center justify-center ${pending || inProgress ? 'bg-warning/10' : passed ? 'bg-success/10' : 'bg-danger/10'}`}>
                      {pending || inProgress ? <Clock className="h-5 w-5 text-warning" /> : passed ? <Trophy className="h-5 w-5 text-success" /> : <XCircle className="h-5 w-5 text-danger" />}
                    </div>
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-1 text-xs text-text-secondary">
                        ...
                        <span>Attempt #{a.attemptNumber || 1}</span>
                        {a.attemptCount > 0 && <span>{a.attemptCount} of {a.isUnlimited ? '∞' : (a.remainingAttempts ?? 0) + a.attemptCount} used</span>}
                        <span className="flex items-center gap-1"><Clock className="h-3 w-3" /> {formatTime(a.totalTimeSpent)}</span>
                        {!pending && !inProgress && <span>{a.correctCount}/{a.correctCount + a.incorrectCount + a.unansweredCount} correct</span>}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    <div className="text-right">
                      {pending ? (
                        <>
                          <p className={`text-lg font-bold ${scoreColor}`}>Awaiting</p>
                          <p className="text-[10px] text-text-tertiary">results not released</p>
                        </>
                      ) : inProgress ? (
                        <>
                        ...
                        <>
                          <p className={`text-xl font-bold ${scoreColor}`}>{a.percentage}%</p>
                          <p className="text-[10px] text-text-tertiary">{a.score}/{a.totalMarks} marks</p>
                        </>
                      )}
                    </div>
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-medium ${badge.cls}`}>
                      {badge.label}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between mt-4 pt-3 border-t border-border">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    {inProgress ? (
                      <Button variant="primary" size="sm" className="h-7 text-xs" onClick={() => navigate(`/assessments/${assessment?._id}`)}>
                    ...
                    {!inProgress && (canRetake ? (
                      ...
                      <span className="text-[10px] px-2 py-1 rounded-full bg-bg-tertiary text-text-tertiary">No attempts left</span>
                    ))}
                  </div>
                </div>
              </div>
